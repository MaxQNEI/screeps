...
import * as ApiAccess from "../api-access.js";
...
export default async function Push(root = import.meta.dirname) {
  ...
  async function direct() {
    return await new Promise(async (resolve, reject) => {
      const auth = ApiAccess.EMAIL && ApiAccess.PASSWORD ? { auth: `${ApiAccess.EMAIL}:${ApiAccess.PASSWORD}` } : null;
      const xtoken = !auth && ApiAccess.XTOKEN ? { "X-Token": ApiAccess.XTOKEN } : null;
